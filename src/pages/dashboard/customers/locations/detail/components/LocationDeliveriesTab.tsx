...
export default function LocationDeliveriesTab({
  customer,
}: {
  customer: Customer;
  ...
  const locationDeliveries = deliveries.filter((d) => d.customer === customer.name);
  ...
  return (
    <div className="rounded-lg border border-background-200 bg-background-50 overflow-hidden">
      <div className="px-4 py-3 border-b border-background-200">
        ...
        <p className="text-[11px] text-foreground-400">{locationDeliveries.length} delivery{locationDeliveries.length === 1 ? '' : 'ies'}</p>
      </div>
      {locationDeliveries.length > 0 ? (
        ...
        <div className="p-12 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-foreground-600">No deliveries for this location</p>
        </div>
      )}
    </div>
  );
