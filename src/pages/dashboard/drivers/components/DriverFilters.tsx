...
import { driverStatuses, type DriverStatus } from '@/mocks/drivers';
...
type DriverFiltersProps = {
  ...
  onStatus: (s: DriverStatus | null) => void;
  search: string;
  onSearch: (s: string) => void;
  truck: string;
  onTruck: (t: string) => void;
  location: string;
  onLocation: (l: string) => void;
  shift: string;
  onShift: (s: string) => void;
  truckOptions: string[];
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-2 pt-2 border-t border-background-200">
        <div className="relative flex-1 min-w-0 lg:max-w-xs">
          ...
          <input
            ...
            placeholder="Search driver, assignment, location…"
