...
const HEALTH_META: Record<RouteHealth, { stroke: string; fill: string; labelKey: string }> = {
    ontime: {
        stroke: 'stroke-green-500',
        fill: 'bg-green-500',
        labelKey: 'dashboard.overview.operationsMap.health.ontime'
    },
    warning: {
        stroke: 'stroke-amber-500',
        fill: 'bg-amber-500',
        labelKey: 'dashboard.overview.operationsMap.health.warning',
    },
    risk: {stroke: 'stroke-red-500', fill: 'bg-red-500', labelKey: 'dashboard.overview.operationsMap.health.risk'},
    scheduled: {
        stroke: 'stroke-gray-400',
        fill: 'bg-gray-400',
        labelKey: 'dashboard.overview.operationsMap.health.scheduled',
    },
};

const FILTERS: { key: Filter; labelKey: string }[] = [
    {key: 'all', labelKey: 'dashboard.overview.operationsMap.filters.all'},
    {key: 'ontime', labelKey: 'dashboard.overview.operationsMap.filters.ontime'},
    {key: 'warning', labelKey: 'dashboard.overview.operationsMap.filters.warning'},
    {key: 'risk', labelKey: 'dashboard.overview.operationsMap.filters.risk'},
];
...
function RunDetails({d}: { d: MapDelivery }) {
    ...
    return (
        <div className="min-w-0">
            <div className="text-[11px] font-semibold text-foreground-900">
                {t('dashboard.overview.operationsMap.details.run')} {d.order}
            </div>
        ...
        <span className="flex items-center gap-1.5">
          <i className="ri-user-line text-foreground-400"/> {t('dashboard.overview.operationsMap.details.driver')}:{' '}
            {d.driver}
        </span>
                <span className="flex items-center gap-1.5">
          <i className="ri-truck-line text-foreground-400"/> {t('dashboard.overview.operationsMap.details.truck')}: {d.truck}
        </span>
                <span className="flex items-center gap-1.5">
          <i className="ri-drop-line text-foreground-400"/> {d.volume} {d.product}
        </span>
                <span className="flex items-center gap-1.5">
          <i className="ri-time-line text-foreground-400"/> {t('dashboard.overview.operationsMap.details.eta')}: {d.eta}
        </span>
                <span className="flex items-center gap-1.5">
          <i className="ri-flag-line text-foreground-400"/> {t('dashboard.overview.operationsMap.details.status')}: {d.status}
        </span>
            </div>
            <span
...
export default function OperationsMap() {
    ...
    const activeFilterLabel = t(activeFilter.labelKey);
    ...
    return (
        <div className="rounded-lg border border-background-200 overflow-hidden relative bg-background-100 h-full">
            <div className="relative h-[440px] md:h-[520px]" onMouseMove={handleMouseMove}>
                <iframe
                    title={t('dashboard.overview.operationsMap.mapTitle')}
                    ...
                    loading="lazy"
                    aria-label={t('dashboard.overview.operationsMap.mapAria')}
                />
                ...
                {terminals.map((t) => (
                    ...
                    >
            <span
                className="w-4 h-4 rounded-full bg-primary-500 flex items-center justify-center ring-2 ring-background-50">
              <i className="ri-building-4-line text-background-50 text-[8px] leading-none"/>
                ...
                {visible.map((d) => {
                    ...
                    return (
                        ...
                        >
              <span
                  className="w-3.5 h-3.5 rounded-full bg-accent-500 flex items-center justify-center ring-2 ring-background-50">
                ...
                {visible.map((d) => {
                    ...
                    return (
                        <button
                            ...
                            aria-label={t('dashboard.overview.operationsMap.selectDelivery', {order: d.order})}
                        >
                            {d.health === 'risk' && (
                ...
                <div className="absolute top-3 left-3 z-10">
                    <div className="relative">
                        ...
                        {filterOpen && (
                            <div
                                className="absolute top-full left-0 mt-1 w-36 bg-background-50 border border-background-200 rounded-md py-1 z-20">
                                {FILTERS.map((f) => (
                                    <button
                                        key={f.key}
                                        onClick={() => {
            ...
            >
              ...
              {t('dashboard.overview.operationsMap.scheduledToggle')}
            </span>
                    </button>
                </div>
                ...
                {selected && (
                    <div
                        className="absolute top-3 right-3 z-20 w-64 bg-background-50/95 border border-background-200 rounded-lg p-3">
                        <div className="flex items-start justify-between">
                            <RunDetails d={selected}/>
                            <button
                                onClick={() => setSelectedId(null)}
                                className="cursor-pointer text-foreground-500 hover:text-foreground-700 ml-2 shrink-0"
                                aria-label={t('dashboard.overview.operationsMap.showAll')}
                            >
                        ...
                        >
                            <i className="ri-arrow-go-back-line text-xs"/> {t('dashboard.overview.operationsMap.showAll')}
                        </button>
                    </div>
                )}
                ...
                {/* Legend */}
                <div
                    className="absolute left-3 bottom-3 bg-background-50/95 rounded-md px-2.5 py-2 flex flex-col gap-1.5 border border-background-200">
          <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <i className="ri-building-4-line text-primary-500 text-[10px] leading-none"/>{' '}
              {t('dashboard.overview.operationsMap.legend.terminal')}
          </span>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <i className="ri-truck-line text-foreground-500 text-[10px] leading-none"/>{' '}
                        {t('dashboard.overview.operationsMap.legend.truck')}
          </span>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <i className="ri-map-pin-2-line text-accent-500 text-[10px] leading-none"/>{' '}
                        {t('dashboard.overview.operationsMap.legend.destination')}
          </span>
                    <span className="h-px bg-background-200 my-0.5"/>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <span className="w-2 h-0.5 bg-green-500 rounded"/> {t('dashboard.overview.operationsMap.health.ontime')}
          </span>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <span className="w-2 h-0.5 bg-amber-500 rounded"/> {t('dashboard.overview.operationsMap.health.warning')}
          </span>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <span className="w-2 h-0.5 bg-red-500 rounded"/> {t('dashboard.overview.operationsMap.health.risk')}
          </span>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-600">
            <span className="w-2 h-0.5 bg-gray-400 rounded"/> {t('dashboard.overview.operationsMap.health.scheduled')}
          </span>
                    <span className="h-px bg-background-200 my-0.5"/>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-500">
            <span
                className="w-3 h-0.5 bg-foreground-400 rounded"/> {t('dashboard.overview.operationsMap.legend.traveled')}
          </span>
                    <span className="flex items-center gap-1.5 text-[9px] text-foreground-500">
            <span className="w-3 border-t border-dashed border-foreground-400"/>{' '}
                        {t('dashboard.overview.operationsMap.legend.remaining')}
          </span>
                </div>

                {/* Live badge */}
                <div
                    className="absolute right-3 bottom-3 bg-background-50/95 rounded-md px-2.5 py-1.5 border border-background-200">
          <span className="text-[9px] font-medium text-foreground-500">
            {t('dashboard.overview.operationsMap.liveBadge', {count: visible.length})}
          </span>
                </div>
            </div>
        </div>
