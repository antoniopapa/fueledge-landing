...
export default function DriverRunPreviewPage() {
  ...
  if (!run) {
    return (
      <DriverAppShell title="Run details" onBack={() => navigate('/driver/home')}>
  ...
  return (
    <DriverAppShell title="Run details" onBack={() => navigate('/driver/home')}>
      <div className="space-y-5">
        ...
        <section>
          <h2 className="mb-3 font-heading text-lg font-bold text-foreground-950">Stops</h2>
          ...
          <>
            {hasActive ? (
              <div className="flex items-center gap-2 rounded-lg border border-background-200 bg-background-100/70 px-4 py-3.5">
                ...
                <span className="text-base font-medium text-foreground-500">Starts after current run</span>
              </div>
            ) : (
              <button
                type="button"
